...
import { Plus, Trash2, Pencil, Landmark, Wallet, Smartphone, CreditCard } from 'lucide-react-native';
...
function getAccountGradient(type: string): [string, string] {
  if (type === 'Cash') return MD3Gradients.teal as [string, string];
  if (type === 'UPI') return MD3Gradients.purple as [string, string];
  if (type === 'Card') return MD3Gradients.delete as [string, string];
  return MD3Gradients.primary as [string, string];
}
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          const gradient = getAccountGradient(item.type);
          return (
            <Animated.View entering={FadeInDown.duration(300).delay(index * 50)} style={styles.card}>
              <LinearGradient
                colors={gradient}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
              ...
              <View style={styles.cardBody}>
                <View style={styles.cardMain}>
                  ...
                  <View style={{ flex: 1 }}>
                    <View style={styles.cardTitleRow}>
                      ...
                      <StatusBadge label={item.type} color={color} bg={bg} />
                    </View>
                    {item.bank_name ? <Text style={styles.cardMeta}>{item.bank_name}</Text> : null}
                ...
                <View style={styles.cardBalanceRow}>
                  <Text style={styles.cardBalanceLabel}>Opening Balance</Text>
                  <Text style={[styles.cardBalanceValue, { color }]}>{formatRs(item.opening_balance)}</Text>
                </View>
              </View>
      ...
      <FAB onPress={() => { setEditing(null); setModalVisible(true); }} icon={Plus} intent="primary" />
...
function AccountFormModal({ visible, account, onClose, onSaved }: { visible: boolean; account: BankAccount | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <PremiumModal
      ...
      footer={
        <>
          <Button title="Cancel" variant="outlined" intent="cancel" onPress={onClose} style={{ flex: 1, marginRight: MD3Spacing.sm }} />
          <Button title={account ? 'Update' : 'Add Account'} intent={account ? 'update' : 'add'} onPress={handleSave} loading={saving} style={{ flex: 1 }} />
        </>
      }
    >
      ...
      <View style={styles.chipRow}>
        {ACCOUNT_TYPES.map(t => {
          ...
          return (
            <TouchableOpacity
              key={t}
              style={[styles.chip, selected && { backgroundColor: getAccountColor(t), borderColor: getAccountColor(t) }]}
              onPress={() => setType(t)}
            >
              <Icon size={14} color={selected ? MD3Colors.onPrimary : getAccountColor(t)} />
              <Text style={[styles.chipText, selected && { color: MD3Colors.onPrimary }]}>{t}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
const styles = StyleSheet.create({
  ...
  cardAccent: { height: 4 },
  cardBody: { padding: MD3Spacing.md },
  cardMain: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: MD3Spacing.sm },
  ...
  cardName: { fontFamily: 'Roboto-Bold', fontSize: 16, color: MD3Colors.onSurface, flex: 1, marginRight: MD3Spacing.sm },
  ...
  cardBalanceLabel: { fontFamily: 'Roboto-Regular', fontSize: 12, color: MD3Colors.onSurfaceVariant },
  cardBalanceValue: { fontFamily: 'Roboto-Bold', fontSize: 16 },
  ...
  chip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: MD3Spacing.md, paddingVertical: MD3Spacing.sm, borderRadius: MD3Radius.full, borderWidth: 1.5, borderColor: MD3Colors.outline, backgroundColor: MD3Colors.surface },
