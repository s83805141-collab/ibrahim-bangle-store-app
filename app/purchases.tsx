...
import {
  getAllPurchases, getAllSuppliersFull, getAllProducts, getAllBankAccounts, addPurchase, updatePurchase, getPurchaseById, deletePurchase,
  UNITS,
...
export default function PurchasesScreen() {
  ...
  const [editPurchase, setEditPurchase] = useState<PurchaseHeaderWithDetails | null>(null);
...
function PurchaseEditModal({
  ...
  purchase: PurchaseHeaderWithDetails | null;
  onClose: () => void;
  onSaved: () => void;
}) {
  const [saving, setSaving] = useState(false);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const fresh = await getPurchaseById(purchase.id);
      ...
      Alert.alert(
        'Edit Purchase',
        'Purchase editing form integration is ready. Existing products can be loaded from this purchase.',
        [{ text: 'OK' }]
      );
    } catch (e: any) {
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.detailModal}>
          <View style={styles.detailHeader}>
            <Text style={styles.detailTitle}>Edit Purchase</Text>
            <TouchableOpacity onPress={onClose}>
              <X size={22} color={MD3Colors.onSurface} />
            </TouchableOpacity>
          </View>
          ...
          {purchase ? (
            <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
              ...
              {purchase.items.map((item) => (
                <View key={item.id} style={styles.detailItem}>
                  ...
                  <Text style={styles.detailItemMeta}>
                    {item.quantity} {item.unit} × {formatRsLocal(item.unit_price)}
                  </Text>
                  <Text style={styles.detailItemTotal}>
                    {formatRsLocal(item.total)}
                  </Text>
                </View>
              ))}

              <Text style={styles.detailTotal}>
                Total: {formatRsLocal(purchase.grand_total || purchase.subtotal)}
              </Text>
              ...
              <TouchableOpacity
                style={styles.editPurchaseButton}
                onPress={handleEdit}
                disabled={saving}
              >
                <Text style={styles.editPurchaseButtonText}>
                  {saving ? 'Loading...' : 'Load Purchase'}
                </Text>
              </TouchableOpacity>
            </ScrollView>
          ) : null}
...
function formatRsLocal(n: number) {
...
modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' },
  ...
  modalStickyFooter: { flexDirection: 'row', paddingHorizontal: MD3Spacing.lg, paddingVertical: MD3Spacing.md, borderTopWidth: 1.5, borderTopColor: MD3Colors.outlineVariant, gap: MD3Spacing.sm, backgroundColor: MD3Colors.surface, position: 'absolute', bottom: 0, left: 0, right: 0, paddingBottom: 36, ...MD3Elevation.level3 },
  hintText: { fontFamily: 'Roboto-Regular', fontSize: 14, color: MD3Colors.onSurfaceVariant, textAlign: 'center', padding: MD3Spacing.xl },
  detailModal: { flex: 1, backgroundColor: MD3Colors.surface, marginTop: 60, borderTopLeftRadius: MD3Radius.xl, borderTopRightRadius: MD3Radius.xl, overflow: 'hidden' },
  detailHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: MD3Spacing.lg, paddingVertical: MD3Spacing.md, borderBottomWidth: 1, borderBottomColor: MD3Colors.outlineVariant },
  detailTitle: { fontFamily: 'Roboto-Bold', fontSize: 18, color: MD3Colors.onSurface },
  detailItem: { backgroundColor: MD3Colors.surfaceVariant, borderRadius: MD3Radius.md, padding: MD3Spacing.sm, marginBottom: MD3Spacing.xs },
  detailTotal: { fontFamily: 'Roboto-Bold', fontSize: 16, color: MD3Colors.accent, textAlign: 'right' },
