...
-- Purchase headers (one per invoice, supports multiple line items)
...
-- Purchase line items (multiple products per purchase header)
...
-- Enhance supplier_ledger with payment details
...
-- Sale headers (one per invoice, supports multiple line items)
...
-- Sale line items (multiple products per sale header)
...
-- Enhance customer_ledger with payment details
...
-- Expand products table with full inventory fields
...
-- Stock movements (audit log for all stock changes: purchase/sale/return/damage/adjustment)
...
-- Expand suppliers with email, gst_number, status
...
-- Expand purchase_headers with discount, transport, other charges
...
-- Supplier payments (multiple payments per purchase)
...
-- Payment proof images (multiple images per payment)
...
-- Bank accounts master (unlimited accounts: Cash, SBI, HDFC, PhonePe, etc.)
...
-- Expand customers with state, status, photo
...
-- Expand sale_headers with discount percent, extra charges, payment date/time, mixed payment
...
-- Customer payments (multiple payments per sale / standalone)
...
-- Customer payment proof images (multiple images per payment)
...
-- Purchase header: payment date/time, UPI, reference, screenshot, partial payment fields
...
-- Purchase items: selling price (for profit tracking)
