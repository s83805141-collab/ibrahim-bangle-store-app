...
export async function updatePurchase(
  ...
  const oldPurchase = await getPurchaseById(headerId);
  if (!oldPurchase) {
  ...
  // Remove old purchase ledger entry.
  await db.exec(
  ...
  const grandTotal = Math.max(
    ...
    header.subtotal -
      (header.discount || 0) +
      (header.transport_charges || 0) +
      (header.other_charges || 0)
  );
  ...
  for (const item of items) {
    await db.exec(
      `INSERT INTO purchase_items
       (purchase_header_id, product_id, variant_id, quantity, unit,
    ...
    await addStockMovement(
      ...
      'purchase',
      headerId,
      `Purchase ${header.invoice_number || ''}`.trim()
    );
  }

  await db.exec(
    `INSERT INTO supplier_ledger
     (supplier_id, type, amount, date, note, payment_method,
      transaction_number, ref_type, ref_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      header.supplier_id,
      'purchase',
  ...
  // Replace old payment records and their proof images.
  const oldPayments = await getPaymentsByPurchase(headerId);

  for (const payment of oldPayments) {
    await db.exec(
      `DELETE FROM payment_proof_images
  ...
  for (const payment of payments) {
    const paymentRes = await db.exec(
      `INSERT INTO supplier_payments
       (supplier_id, purchase_header_id, amount, payment_date,
        payment_time, payment_mode, bank_account_id, bank_name,
        account_name, account_number, upi_id, transaction_number,
        cheque_number, reference_number, note, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        header.supplier_id,
        headerId,
    ...
    for (const imgPath of payment.proof_images || []) {
      await db.exec(
        `INSERT INTO payment_proof_images
         (supplier_payment_id, image_path, caption, created_at)
         VALUES (?, ?, ?, ?)`,
        [paymentId, imgPath, '', now]
      );
    }
    ...
    await db.exec(
      ...
      [
        ...
        payment.amount,
        payment.payment_date,
        `Payment for ${header.invoice_number || ''}`.trim(),
        payment.payment_mode,
        payment.transaction_number || '',
        'payment',
        paymentId,
...
export interface QuickBillHeader {
  id?: number;
  bill_number: string;
  created_at: number;
  total_amount: number;
...
export async function addQuickBill(
  header: { total_amount: number; item_count: number },
  items: QuickBillItem[]
): Promise<{ id: number; bill_number: string }> {
  const db = await getDb();

  const res = await db.exec(
    `INSERT INTO quick_bill_headers
      (bill_number, created_at, total_amount, item_count)
     VALUES (?, ?, ?, ?)`,
    ['', Date.now(), header.total_amount || 0, header.item_count || 0]
  );

  const id = res.insertId ?? 0;

  if (!id) {
    throw new Error('Failed to insert quick bill header');
  }

  const billNumber = `CB-${String(id).padStart(5, '0')}`;

  await db.exec(
    `UPDATE quick_bill_headers SET bill_number = ? WHERE id = ?`,
    [billNumber, id]
  );

  for (const item of items) {
    await db.exec(
      `INSERT INTO quick_bill_items
        (quick_bill_id, product_id, product_name, quantity, unit_price, line_total)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        id,
...
export async function getQuickBills(
  limit = 25
): Promise<QuickBillHeader[]> {
  const db = await getDb();

  const res = await db.exec(
    `SELECT id, bill_number, created_at, total_amount, item_count
     FROM quick_bill_headers
     ORDER BY created_at DESC, id DESC
     LIMIT ?`,
    [limit]
  );

  return (res.rows._array || []) as QuickBillHeader[];
}

export async function getQuickBillById(
  id: number
): Promise<{ header: QuickBillHeader; items: QuickBillItem[] } | null> {
  const db = await getDb();

  const headerRes = await db.exec(
    `SELECT id, bill_number, created_at, total_amount, item_count
     FROM quick_bill_headers
     WHERE id = ?`,
    [id]
  ...
  const header = headerRes.rows._array[0] as QuickBillHeader;

  const itemsRes = await db.exec(
    `SELECT id, quick_bill_id, product_id, product_name,
            quantity, unit_price, line_total
     FROM quick_bill_items
     WHERE quick_bill_id = ?
     ORDER BY id`,
    [id]
  );
  ...
  return {
    header,
    items: (itemsRes.rows._array || []) as QuickBillItem[],
  };
}

export async function pruneQuickBills(limit = 25): Promise<void> {
  const db = await getDb();

  const keepRes = await db.exec(
    `SELECT id
     FROM quick_bill_headers
     ORDER BY created_at DESC, id DESC
     LIMIT ?`,
  ...
  const keepIds = (keepRes.rows._array || []).map(
  ...
  await db.exec(
    `DELETE FROM quick_bill_headers
